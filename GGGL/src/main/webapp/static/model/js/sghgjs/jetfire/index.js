...
function jetfire(e){
	top.layer.open({
        ...
        maxmin: false, 
        shift: 1,
	    content: ctx+'/sghgjs/jetfire/create',
	    btn: ['计算', '取消'],
	    yes: function(index, layero){
	    	 var body = top.layer.getChildFrame('body', index);
	         var iframeWin = layero.find('iframe')[0]; 
	         var inputForm = body.find('#jetfire_form');
	         
	         body.find("#jetfire_form_id_m11").val(e.point.lng);
	         body.find("#jetfire_form_id_m12").val(e.point.lat);
	         
	         if(iframeWin.contentWindow.doSubmit()){
	        	//显示应急资源、疏散距离、处理技术
	     		var postdata={wzname: body.find('#jetfire_form_id_m1_1').val(), lng:e.point.lng , lat:e.point.lat }
	     		show(postdata);
	     		
	     		//计算事故结果
	     		$.ajax({
	    			type:'post',
	    			url:ctx+"/sghgjs/jetfire/create",
