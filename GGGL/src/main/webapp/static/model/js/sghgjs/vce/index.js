...
function vce(e){
	top.layer.open({
        ...
        maxmin: false, 
        shift: 1,
	    content: ctx+'/sghgjs/vce/create',
	    btn: ['计算', '取消'],
	    yes: function(index, layero){
	    	 var body = top.layer.getChildFrame('body', index);
	         var iframeWin = layero.find('iframe')[0]; 
	         var inputForm = body.find('#vce_form');
	         
	         body.find("#vce_form_id_m6").val(e.point.lng);
	         body.find("#vce_form_id_m7").val(e.point.lat);
	         
	         if(iframeWin.contentWindow.doSubmit()){
	        	//显示应急资源、疏散距离、处理技术
	     		var postdata={wzname:body.find('#vce_form_id_m1_1').val(), lng:e.point.lng , lat:e.point.lat }
	     		show(postdata);
	     		
	     		//计算事故结果
	     		$.ajax({
	    			type:'post',
	    			url:ctx+"/sghgjs/vce/create",
