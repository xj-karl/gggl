...
var Menu = function(options){
	this.cacheOpen = {};
	this.nav2 = options.nav2 || $('.nav2');
	this.tab = options.tab || $('.tab');
	this.content = options.content || $('.content');	
	this.defaultSelect = options.defaultSelect || this.nav2.find('li[data-id="1"]');
...
Menu.prototype._createContent = function($this){
	...
	var $iframe= $('<iframe name="myiframe" data-id="'+ id +'" width="100%" height="100%" frameborder="0" src="'+ href +'"></iframe>');
	//设置iframe高度，32为tab高度
	$iframe.height($("#bd").height() - 32);
	 
	this.cacheOpen[id]={nav2:$this, tab:$tab, iframe:$iframe};
...
Menu.prototype._show = function($this){
	...
	this.nav2.find('li').removeClass('current');
	this.nav2.find('li[data-id="'+ id +'"]').addClass('current').parents('.nav-li').siblings().find(".subnav").hide().end().end().addClass('current').find(".subnav").show();
...
Menu.prototype._close = function($this, flag){
	...
	/*if(current && current.nav2.hasClass('current')){ // modify 2014-3-24 can close the current
		return;	
	}*/
	// 执行关闭
	current.tab.remove();
	...
	if(current && current.nav2.hasClass('current')){ // modify 2014-3-24 delete current the prev one is selected
		this._show($tab);
	}
...
Menu.prototype._bindEvent = function(){
	...
	var self = this;
	this.nav2.on('click', 'li', function(){
	...
	this.nav2.on('click', '.subnav li', function(){
		self._select($(this));
	});
	
	this.nav2.on('click', '.nav-li', function(){
		self._select($(this));
	});
