...
//打开对话框(查看)
function openDialogView(title, url, width, height, options) {
    layer.open($.extend({
        type: 2,
        shift: 1,
        area: [width, height],
...
function openImgView(imgurl) {
    top.layer.open({
        ...
        closeBtn: 0,
        shadeClose: true,
        content: '<img border=0 src=' + imgurl + ' style="width:100%;">'
    });
}
...
// 初始化富文本
//domname:dom id值 ，height 高度
function onloadEditor(domname, height, width) {
    ...
    // 渲染富文本
    window.editor = KindEditor.create('#' + domname, $.extend({},
        KEditorConfig, {
