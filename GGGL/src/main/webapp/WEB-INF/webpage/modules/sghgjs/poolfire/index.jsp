...
<head>
	...
	<script type="text/javascript" src="${ctxStatic}/model/js/sghgjs/poolfire/index.js?v=1.1"></script>
    <script type="text/javascript" src="${ctx}/static/common/initmap.js?v=1.1"></script> 
	<style type="text/css">
	.BMap_cpyCtrl{ display:none; }  
    .anchorBL{ display:none;}   
    #poolfiredituContent label{max-width:none;}
...
<div id="poolfire_yjzy_win" style="display: none;width: 100%;height: 100%;">
	...
	<div id="poolfire_yjcl_win"  style="display: none;">
		<table id="poolfire_yjcl_dg" class="yjcl_table">
	...
	<div id="poolfire_ssjl_win" style="display: none;">
		<table id="poolfire_ssjl_dg" class="yjcl_table" style="text-align: center;">
...
//初始化地图	
initMap("poolfiredituContent", qylng, qylat,16);

function showDatagrid(postdata){
	var divwidth = $("#poolfiredituContent").width()/2;
	if(divwidth>350) divwidth=350;
	var left= $(window).width()-divwidth;
	var height = $("#poolfiredituContent").height();
	...
	layer.open({
        ...
        shift: 5,
        shade :0,
	    content: $('#poolfire_ssjl_win')
	});

	$('#poolfire_yjzy_dg').datagrid({     
		 method: "post",
...
function show(postdata){
	if(flag){
		$('#poolfire_yjzy_dg').datagrid('load',postdata );	
		getData(postdata);
	}else{
