...
<head>
	...
	<script type="text/javascript" src="${ctxStatic}/model/js/sghgjs/fireball/index.js?v=1.1"></script>
    <script type="text/javascript" src="${ctx}/static/common/initmap.js?v=1.1"></script> <script type="text/javascript" src="${ctx}/static/common/initmap.js?v=1.1"></script> 
	<style type="text/css">
	.BMap_cpyCtrl{ display:none; }  
    .anchorBL{ display:none;}   
    #fireballdituContent label{max-width:none;}
...
<body>
<div style="height:100%;width:100%;border:#ccc solid 1px;position:absolute;" id="fireballdituContent"></div>


<div id="fireball_yjzy_win" style="display: none;width: 100%;height: 100%;">
	<table id="fireball_yjzy_dg"> </table>
</div>
<div id="fireball_yjcl_win" style="display: none;">
	<table id="fireball_yjcl_dg" class="yjcl_table" > 
...
<div id="fireball_ssjl_win"  style="display: none;">
	<table id="fireball_ssjl_dg" class="yjcl_table" style="text-align: center;">
...
//初始化地图	
initMap("fireballdituContent", qylng, qylat,16);

function showDatagrid(postdata){
	var divwidth = $("#fireballdituContent").width()/2;
	if(divwidth>350) divwidth=350;
	var left= $(window).width()-divwidth;
	var height = $("#fireballdituContent").height();
	...
	layer.open({
        ...
        shift: 5,
        shade :0,
	    content: $('#fireball_ssjl_win')
	});
	
	$('#fireball_yjzy_dg').datagrid({     
		 method: "post",
...
function show(postdata){
	if(flag){
		$('#fireball_yjzy_dg').datagrid('load',postdata );	
		getData(postdata);
	}else{
